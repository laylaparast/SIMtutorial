...
This is an R markdown file (extension is .Rmd) to accompany "Surrogate Marker Evaluation: A Tutorial Using R" by Layla Parast. If you have any trouble with this code, file, or packages, please email me at parast\@austin.utexas.edu and I will help you.
...
You will need to install the packages below before you run anything else or try to knit this file; that is, press the small green right-pointing arrow in the top right corner of the box. Because the **SurrogateParadoxTest** package is not yet on CRAN, we need to install it from Github using the **devtools** package.
...
#this package needs to be installed from Github using devtools
install.packages("devtools")
library(devtools)
devtools::install_github("emily13hsiao/SurrogateParadoxTest",
    subdir = "SurrogateParadoxTest", build = FALSE)
```
...
number = "multiple", type = "freedman", var = TRUE, conf.int = TRUE)
```
...
R.s.estimate(yone=d_example$y1, yzero=d_example$y0, sone=cbind(d_example$s1.a,d_example$s1.b, 
d_example$s1.c), szero=cbind(d_example$s0.a, d_example$s0.b, d_example$s0.c), 
number = "multiple", type = "model", var = TRUE, conf.int = TRUE)
...
R.s.estimate(yone=d_example$y1, yzero=d_example$y0, sone=cbind(d_example$s1.a,d_example$s1.b, 
d_example$s1.c), szero=cbind(d_example$s0.a, d_example$s0.b, d_example$s0.c), 
number = "multiple", type = "robust", var = TRUE, conf.int = TRUE)
```
...
me.variance = var.u, naive= TRUE, Ronly = TRUE)
```
...
set.seed(4)
R.s.surv.estimate(xone = d_example_surv$x1, xzero =d_example_surv$x0, deltaone =d_example_surv$delta1, deltazero = d_example_surv$delta0, sone = d_example_surv$s1, szero=d_example_surv$s0, t=2, 
landmark = 1, var=TRUE, conf.int=TRUE)
```
...
=d_example_surv$delta1, deltazero = d_example_surv$delta0, sone = d_example_surv$s1, 
szero=d_example_surv$s0, t=2, landmark = 1, basis.delta.one = d_example_surv$z1 , basis.delta.zero =
d_example_surv$z0)
...
d_example_multiple$delta1, deltazero = d_example_multiple$delta0, sone =  as.matrix(d_example_multiple$s1), szero = as.matrix(d_example_multiple$s0), t = 1, landmark=0.5, var=TRUE, conf.int=TRUE)
```
